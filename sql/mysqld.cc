...
#include <sys/mman.h>
#if defined(__sun__) && defined(__GNUC__) && defined(__cplusplus) && defined(_XOPEN_SOURCE)
...
#define _FPU_GETCW(cw) asm volatile("fnstcw %0" \
                                    : "=m"(*&cw))
#define _FPU_SETCW(cw) asm volatile("fldcw %0" \
                                    :          \
                                    : "m"(*&cw))
#else
...
static const char *rocksdb_engine_name = "rocksdb_rpc";
...
static MYSQL_SOCKET activate_tcp_port(uint port, my_bool socket_sharding = 0)
{
  ...
  */
  ...
  {
    if (((ret = mysql_socket_bind(tmp_ip_sock, a->ai_addr, a->ai_addrlen)) >= 0) || (socket_errno != SOCKET_EADDRINUSE) ||
...
static bool block_until_new_connection_halflock()
{
      ...
      !kill_blocked_pthreads_flag.load(std::memory_order_relaxed))
  {
    ...
    */
    ...
    // Block pthread
    bool kbpf = false;
    while (!(kbpf = kill_blocked_pthreads_flag.load(std::memory_order_acquire)) && (!abort_loop) && (!wake_pthread))
    {
...
static int latency_histogram_bin_search(latency_histogram *current_histogram,
                                        ulonglong value)
{
  if (current_histogram->step_size == 0 || value == 0 || current_histogram->step_ratio <= 0.0)
    return -1;
...
int histogram_validate_step_size_string(const char *step_size_with_unit)
{
  ...
  {
    if (strcmp(histogram_unit, "ms") && strcmp(histogram_unit, "us") && strcmp(histogram_unit, "s"))
      ret = 1;
  }
  /* Special case when step size is passed to be '0' */
                                    ...
                                    ulonglong *histogram_values)
{
  ...
  for (i = 0, bucket_lower_display = 0; i < NUMBER_OF_HISTOGRAM_BINS; ++i)
  {
    bucket_upper_display =
        my_timer_to_microseconds_ulonglong(itr_step_size) + bucket_lower_display;
...
SHOW_VAR com_status_vars[] = {
    ...
    {"attach_explicit_snapshot", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_ATTACH_EXPLICIT_SNAPSHOT]), SHOW_LONG_STATUS},
    ...
    {"create_explicit_snapshot", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_CREATE_EXPLICIT_SNAPSHOT]), SHOW_LONG_STATUS},
    ...
    {"purge_gtid", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_PURGE_UUID]), SHOW_LONG_STATUS},
    {"rbr_column_names", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_RBR_COLUMN_NAMES]), SHOW_LONG_STATUS},
    {"release_savepoint", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_RELEASE_SAVEPOINT]), SHOW_LONG_STATUS},
    {"release_explict_snapshot", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_RELEASE_EXPLICIT_SNAPSHOT]), SHOW_LONG_STATUS},
    ...
    {"show_raftstatus", (char *)offsetof(STATUS_VAR, com_stat[(uint)SQLCOM_SHOW_RAFT_STATUS]), SHOW_LONG_STATUS},
...
#define setup_fds(X) \
  ...
  while (!abort_loop)
  {
    ...
    if (!admin && separate_conn_handling_thread)
    {
      ...
      pkt.is_admin = admin;
      pkt.is_unix_sock = (mysql_socket_getfd(sock) == mysql_socket_getfd(unix_sock));
  ...
  */
     ...
     easier to do */
        ...
        {"port-open-timeout", 0,
         ...
         "(Default: No wait).",
         &mysqld_port_timeout, &mysqld_port_timeout, 0,
...
{
  update_malloc_status();
  return show_jemalloc_sizet(thd, var, buff, "stats.arenas." JEMALLOC_STRINGIFY(MALLCTL_ARENAS_ALL) ".tcache_bytes");
}
  ...
  if (enable_profiling)
  {
    ...
    if (ret != 0)
    {
      ...
      sql_print_information("Unable to enable "
                            "jemalloc heap profiling. Error:  %d",
                            ret);
      return false;
    }
    return true;
...
static int show_slave_dependency_begin_waits(THD *thd, SHOW_VAR *var, char *buff)
{
  if (active_mi && active_mi->rli && active_mi->rli->mts_dependency_replication)
  {
    var->type = SHOW_LONGLONG;
...
static int show_slave_rows_last_search_algorithm_used(THD *thd, SHOW_VAR *var, char *buff)
{
  ...
  const char *s = ((res == Rows_log_event::ROW_LOOKUP_TABLE_SCAN) ? "TABLE_SCAN" : ((res == Rows_log_event::ROW_LOOKUP_HASH_SCAN) ? "HASH_SCAN" : "INDEX_SCAN"));
...
static int generate_apply_file_gvars()
{
  ...
  {
    sql_print_error("Path '%s' is a directory name, please specify \
a file name for --apply-log option",
                    opt_apply_logname);
    unireg_abort(1);
  }
  ...
  if (opt_applylog_index_name &&
      opt_applylog_index_name[strlen(opt_applylog_index_name) - 1] == FN_LIBCHAR)
  {
    sql_print_error("Path '%s' is a directory name, please specify \
a file name for --apply-log-index option",
...
static int get_options(int *argc_ptr, char ***argv_ptr, my_bool logging)
{
  ...
  */
  ...
  global_system_variables.lock_wait_timeout_nsec = (ulonglong)(global_system_variables.lock_wait_timeout_double * 1e9);
...
bool setup_datagram_socket(sys_var *self, THD *thd, enum_var_type type)
{
  ...
  if (log_datagram)
  {
    ...
    // set nonblocking
    if (fcntl(log_datagram_sock, F_SETFL, O_NONBLOCK | fcntl(log_datagram_sock, F_GETFL)) == -1)
    {
      log_datagram = 0;
      close(log_datagram_sock);
      log_datagram_sock = -1;
